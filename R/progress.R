...
#'
#' @description Show progress while a timer is running following a call of tick(). To suppress all progress updates,
#' set the environment variable .cooltools.env$suppressProgress to \code{TRUE}.
#'
...
progress = function(txt=NULL) {
  ...
  if (is.null(.cooltools.env$timerRunning) || !.cooltools.env$timerRunning) {
    ...
    if (is.null(.cooltools.env$suppressProgress) || !.cooltools.env$suppressProgress) {
