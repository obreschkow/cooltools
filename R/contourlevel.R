...
#' smoothcontour(g$x,g$y,g$n,levels=l,smoothing = 0.7,col='red',lwd=c(2,1))
#'
#' @export
...
.contourlevel.function = function(f, p, xmin=NULL, xmax=NULL, neval=1e4, napprox=10, ...) {
  ...
  f.max = optim((xmin+xmax)/2,f,lower=xmin,upper=xmax,method='L-BFGS-B',
  ...
  for (i in seq(n)) {
    ...
    l[i] = optimize(difference,c(0,f.max))$minimum
  }
